...
describe('sao chép lịch sang người khác', () => {
  async function copy(
    session: Session,
    fromMemberId: string,
    toMemberId: string,
    ...
    return call(
      '/api/activities/copy',
      { method: 'POST', body: json({ fromMemberId, toMemberId }) },
      session.cookie,
    );
  }

  it('chép mọi hoạt động, giữ nguyên giờ và các thứ', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    await addActivity(s, {
      ...BASE,
      memberId,
      title: 'Học thêm Anh',
      kind: 'study',
      location: 'Trung tâm ABC',
      daysOfWeek: [2, 4],
      startTime: '18:30',
      endTime: '20:00',
      effectiveFrom: '2026-09-01',
    ...
    await addActivity(s, { ...BASE, memberId, title: 'Học bơi', daysOfWeek: [7] });

    const res = await copy(s, memberId, em);
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({ copied: 2, fromName: 'Mẹ Lan', toName: 'Bé Nam' });

    const mine = await listActivities(s, `?memberId=${em}`);
    expect(mine).toHaveLength(2);
    expect(mine.find((a) => a.title === 'Học thêm Anh')).toMatchObject({
      memberId: em,
      memberName: 'Bé Nam',
      kind: 'study',
      location: 'Trung tâm ABC',
  ...
  it('bản chép là hàng mới, người gốc giữ nguyên lịch', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    const original = await addActivity(s, { ...BASE, memberId });

    await copy(s, memberId, em);
    const source = await listActivities(s, `?memberId=${memberId}`);
    const target = await listActivities(s, `?memberId=${em}`);
    expect(source).toHaveLength(1);
    expect(source[0].id).toBe(original);
    expect(target[0].id).not.toBe(original);
  });

  it('chép ca qua đêm vẫn giữ đúng độ dài', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bố Nam', color: 'c1' });
    await addActivity(s, { ...BASE, memberId, title: 'Ca đêm', startTime: '22:00', endTime: '06:00' });

    await copy(s, memberId, em);
    expect((await listActivities(s, `?memberId=${em}`))[0]).toMatchObject({
      startTime: '22:00',
      endTime: '06:00',
      durationMin: 480,
      overnight: true,
  ...
  it('cộng thêm vào lịch sẵn có chứ không đè lên', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    await addActivity(s, { ...BASE, memberId, title: 'Của mẹ' });
    await addActivity(s, { ...BASE, memberId: em, title: 'Của bé' });

    expect(((await (await copy(s, memberId, em)).json()) as { copied: number }).copied).toBe(1);
    expect((await listActivities(s, `?memberId=${em}`)).map((a) => a.title).sort()).toEqual([
      'Của bé',
      'Của mẹ',
    ]);
  });

  it('không chép hoạt động đã xoá mềm', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    const gone = await addActivity(s, { ...BASE, memberId, title: 'Đã bỏ' });
    await addActivity(s, { ...BASE, memberId, title: 'Còn dùng' });
    await call(`/api/activities/${gone}`, { method: 'DELETE' }, s.cookie);

    expect(((await (await copy(s, memberId, em)).json()) as { copied: number }).copied).toBe(1);
    expect((await listActivities(s, `?memberId=${em}`)).map((a) => a.title)).toEqual(['Còn dùng']);
  });

  it('KHÔNG chép ngoại lệ — nghỉ hay dời là chuyện riêng của người gốc', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    const id = await addActivity(s, { ...BASE, memberId }); // T2/T4/T6 18:00-20:00
    await call(
      `/api/activities/${id}/exceptions`,
      { method: 'POST', body: json({ occursOn: '2026-09-02', status: 'cancelled' }) },
      s.cookie,
    ...
    await copy(s, memberId, em);
    const copied = (await listActivities(s, `?memberId=${em}`))[0];
    const res = await call(`/api/activities/${copied.id}/exceptions`, {}, s.cookie);
    expect(((await res.json()) as { exceptions: unknown[] }).exceptions).toEqual([]);

    // Người gốc vẫn nghỉ buổi đó, người nhận thì không.
    const week = '?from=2026-08-31&to=2026-09-06';
    const sched = (await (await call(`/api/schedule${week}`, {}, s.cookie)).json()) as {
      occurrences: Array<{ memberId: string; date: string }>;
    };
    const on02 = sched.occurrences.filter((o) => o.date === '2026-09-02');
    expect(on02.map((o) => o.memberId)).toEqual([em]);
  });

  it('lịch rỗng thì chép được nhưng không ra gì', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    expect(((await (await copy(s, memberId, em)).json()) as { copied: number }).copied).toBe(0);
  });

  it('không chép sang chính mình', async () => {
    const { s, memberId } = await setup();
    const res = await copy(s, memberId, memberId);
    expect(res.status).toBe(400);
    expect(((await res.json()) as { error: string }).error).toContain('chính người đó');
  });

  it('người không tồn tại hoặc thuộc hộ khác đều là 404', async () => {
    const { s, memberId } = await setup();
    const b = await registerOwner('b@example.com', 'Nhà B');
    const outsider = await addMember(b, { name: 'Người nhà B' });

    expect((await copy(s, memberId, 'khong-co-that')).status).toBe(404);
    expect((await copy(s, 'khong-co-that', memberId)).status).toBe(404);
    expect((await copy(s, memberId, outsider)).status).toBe(404);
    // Và hộ B không hề nhận được gì.
    expect(await listActivities(b, `?memberId=${outsider}`)).toEqual([]);
  });

  it('người đã xoá mềm không nhận và không cho chép', async () => {
    const { s, memberId } = await setup();
    const em = await addMember(s, { name: 'Bé Nam', color: 'c3' });
    await call(`/api/family-members/${em}`, { method: 'DELETE' }, s.cookie);
    expect((await copy(s, memberId, em)).status).toBe(404);
    expect((await copy(s, em, memberId)).status).toBe(404);
  });

  it('đòi đăng nhập', async () => {
    expect(
      (await call('/api/activities/copy', { method: 'POST', body: json({ fromMemberId: 'a', toMemberId: 'b' }) }))
        .status,
    ).toBe(401);
  });
});
